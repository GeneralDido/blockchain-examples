...
contract CustomToken is ERC20Upgradeable, OwnableUpgradeable {
    ...
    function initialize(
        ...
        uint8 stakePrecision, address contract_address
    ) public initializer
    {
        __Ownable_init();
    ...
    function _stake(address sender, uint256 amount) public {
    ...
    // Any required constrains and checks should be coded as well.
    function _unstake(address sender) public {
    ...
    // This method should allow withdrawing accumulated reward for all staked funds of the user's.
    // Any required constrains and checks should be coded as well.
    // Important! Withdrawing reward should not decrease the stake, stake should be rolled over for the future automatically.
    function _reward(address _address) public {
